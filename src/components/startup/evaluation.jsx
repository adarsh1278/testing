...
export const columns = [
...

    {
        ...
        cell: ({ row }) => <div className="lowercase ">{row.getValue("evaluatorName")}</div>,
    },

    {
        ...
        cell: ({ row }) => <div className="capitalize   ">{row.getValue("evaluationDate")}</div>,
    },
    {
        accessorKey: "status",
        header: ({ column }) => (
...

 {
    ...
    cell: ({ row }) => (
        <div className="text-left h-8 ml-6 flex gap-2">
            <button onClick={() => console.log(`View item with id: ${row.original.id}`)}>
                <span className="text-white bg-blue-800  rounded-lg p-2 hover:text-white">View</span>
            </button>
